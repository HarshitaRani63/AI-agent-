...
def find_catalog_item_by_id_db(item_id: str) -> Optional[dict]:
    ...
    cur = conn.cursor()
    cur.execute(
        "SELECT * FROM catalog WHERE LOWER(id) = LOWER(?) LIMIT 1",
        (item_id,),
    )
    row = cur.fetchone()
    conn.close()
...
async def simulate_delivery_flow(order_id: str):
    ...
    for next_status in STATUS_FLOW[1:]:
        ...
        if curr_order and curr_order.get("status") == "cancelled":
            logger.info(
                f"🛑 [Simulation] Order {order_id} was cancelled. Stopping simulation."
            )
            return
...
async def find_item(
    ...
    query: Annotated[
        str, Field(description="Name or partial name of item (e.g., 'milk', 'paneer')")
    ],
) -> str:
...
async def add_to_cart(
    ...
    for ci in ctx.userdata.cart:
        if ci.item_id.lower() == item_id.lower():
            ...
            return (
                f"Updated '{ci.name}' quantity to {ci.quantity}. "
    ...
    total = cart_total(ctx.userdata.cart)
    return (
        f"Added {quantity} x '{item['name']}' to cart. "
        f"Cart total: ₹{total:.2f}"
    )
...
async def show_cart(ctx: RunContext[Userdata]) -> str:
    ...
    for ci in ctx.userdata.cart:
        lines.append(
            f"- {ci.quantity} x {ci.name} @ ₹{ci.unit_price:.2f} each = ₹{ci.unit_price * ci.quantity:.2f}"
        )
    total = cart_total(ctx.userdata.cart)
    return "Your cart:\n" + "\n".join(lines) + f"\nTotal: ₹{total:.2f}"
...
async def add_recipe(
    ctx: RunContext[Userdata],
    dish_name: Annotated[
        str, Field(description="Name of dish, e.g. 'chai', 'maggi', 'dal chawal'")
    ],
) -> str:
    ...
    total = cart_total(ctx.userdata.cart)
    return (
        f"Added ingredients for '{dish_name}': {', '.join(added)}. "
        f"Cart total: ₹{total:.2f}"
    )
    ...
    request: Annotated[
        ...
        Field(
            description=(
                ...
                "'ingredients for peanut butter sandwich for two'"
            )
        ),
    ],
) -> str:
    """Handle high-level ingredient requests like 'ingredients for peanut butter sandwich'
    or 'get me pasta for two people'. Attempts a map lookup first, then falls back to tag inference.
    """
...
async def place_order(
    ...
    try:
        ...
        # Fallback: if no running event loop, just log it
        logger.warning(
            "No running event loop to schedule delivery simulation for %s",
            order_id,
        )

    return (
        f"Order placed successfully! Order ID: {order_id}. "
        f"Total: ₹{total:.2f}. I have enabled express tracking; "
        "the status will update automatically over time."
    )
...
async def get_order_status(
    ...
    if not o:
        return f"No order found with id {order_id}."
    return (
        f"Order {order_id} status: {o.get('status', 'unknown')}. "
        f"Updated at: {o.get('updated_at')}"
    )
...
async def order_history(
    ...
    customer_name: Annotated[
        Optional[str],
        Field(description="Optional customer name to filter", default=None),
    ] = None,
) -> str:
    ...
    for o in rows:
        lines.append(
            f"- {o['order_id']} | ₹{o['total']:.2f} | Status: {o.get('status')}"
        )
...
class FoodAgent(Agent):
    def __init__(self):
        super().__init__(
            instructions="""
            You are 'Robin', a helpful voice assistant for an Indian grocery store.
            Currency is Indian Rupees (₹).

            Capabilities:
            1. Catalog: Search for Indian items (Amul milk, Tata salt, Maggi, Basmati rice).
            2. Cart: Add or remove items, update quantities, and show cart details.
            3. Recipes: Add ingredients for dishes like Chai, Maggi, and Paneer Butter Masala.
            4. Orders: Place orders with express tracking enabled.
            5. Cancellation: Allow users to cancel an order if it is not yet delivered.

            When placing an order, mention that express tracking is enabled.
            If the user asks "Where is my order?", check the status via the tools.
            The order status advances automatically in the background over time.
            """,
            tools=[
                find_item,
                add_to_cart,
...
async def entrypoint(ctx: JobContext):
    ...
    logger.info("\n" + "🇮🇳" * 12)
    logger.info("🚀 STARTING GROCERY VOICE AGENT (Indian Context + Auto-Tracking)")
